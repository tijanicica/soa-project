...
import { startTour } from "../services/TourExecutionApi";
...
export function MyPurchasedToursPage() {
  ...
  const handleStartTour = async (tourId) => {
    try {
      ...
      if (!positionData.latitude.Valid || !positionData.longitude.Valid) {
        alert("Please set your position in the Position Simulator first.");
        navigate("/simulator"); // Preusmeri korisnika da postavi poziciju
        return;
      }

      const startPosition = {
        Latitude: positionData.latitude.Float64,
        Longitude: positionData.longitude.Float64,
      };

      // 2. Pozovi backend da kreira sesiju
      const execution = await startTour(tourId, startPosition);

      // 3. Preusmeri na stranicu za aktivnu turu
      navigate(`/active-tour/${execution.id}`);
    } catch (error) {
      console.error("Failed to start tour:", error);
      alert(error.response?.data?.message || "Could not start the tour.");
    }
  };
  ...
  useEffect(() => {
    const fetchMyTours = async () => {
      ...
      try {
        ...
        setError(
          "Could not load your purchased tours. Please try again later."
        );
      ...
      <main className="container max-w-7xl mx-auto py-8 md:py-12">
        <div className="mb-10">
          ...
          <p className="text-muted-foreground mt-2">
            Here you can find all the adventures you've unlocked.
          </p>
        </div>
            ...
            {purchasedTours.length > 0 ? (
                  ...
                  >
                    ...
                    <CardHeader>
                      ...
                      <CardDescription className="pt-2 flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        Purchased on:{" "}
                        {new Date(tour.PurchaseDate).toLocaleDateString()}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="flex-grow space-y-4">
                      <div className="space-y-3 text-sm">
                        ...
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <span>
                            Price: <strong>{tour.Price.toFixed(2)} RSD</strong>
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <span>
                            Distance:{" "}
                            <strong>{tour.DistanceKm.toFixed(1)} km</strong>
                          </span>
                        </div>
                        {tour.TransportTimes &&
                      ...
                      <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value="item-1">
                          <AccordionTrigger>
                            Key Points ({tour.KeyPoints.length})
                          </AccordionTrigger>
                          <AccordionContent>
                              ...
                              {tour.KeyPoints.map((kp, index) => (
                                <div key={index} className="border-t pt-4">
                                  <h4 className="font-semibold flex items-center gap-2">
                                    <MapPin className="h-4 w-4" /> {kp.Name}
                                  </h4>
                                  {kp.ImageUrl ? (
                                    <img
                                      src={kp.ImageUrl}
                                      alt={kp.Name}
                                      className="w-full h-auto max-h-48 object-cover rounded-md my-2 border"
                                    />
                                  ) : (
              ...
              <div className="text-center py-16 border-2 border-dashed rounded-lg">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  You haven't purchased any tours. Let's find your next one!
                </p>
